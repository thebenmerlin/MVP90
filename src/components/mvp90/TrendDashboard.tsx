...
const TrendDashboard: React.FC<TrendDashboardProps> = ({ userRole }) => {
  ...
  const getMomentumColor = (momentum: number) => {
    if (momentum >= 80) return "text-[var(--positive)]";
    if (momentum >= 60) return "text-[var(--warning)]";
  ...
  return (
    <div className="space-y-6 overflow-auto h-full pr-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[22px] font-semibold text-foreground">Trend Dashboard</h2>
          <p className="text-[13px] text-muted-foreground">Market analytics and sector intelligence</p>
        </div>
        <div className="flex items-center gap-3">
          {isRefreshing && (
            <span className="text-[12px] text-muted-foreground">Refreshing…</span>
          )}
          <select
            value={timeRange}
            onChange={(e) => setTimeRange(e.target.value as "7d" | "30d" | "90d")}
            className="text-[12px] border border-border bg-background rounded h-8 px-2 text-foreground"
          >
      ...
      {/* Sector Momentum */}
      <div>
        <h3 className="text-[13px] font-semibold text-foreground mb-3">Sector Momentum</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {sectorData.map((sector) => (
            <div key={sector.sector} className="bg-card border border-border rounded p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-[14px] font-semibold text-foreground">{sector.sector}</h4>
                <span className={`text-[24px] font-mono font-semibold ${getMomentumColor(sector.momentum)}`}>
                  {sector.momentum.toFixed(0)}%
                </span>
              </div>

              <div className="w-full bg-muted rounded-full h-[3px] mb-3">
                <div
              ...
              <div className="flex items-center justify-between text-[12px] text-muted-foreground">
                <span>{sector.dealCount} deals · {sector.totalFunding}</span>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[13px] font-semibold text-foreground">Top Scoring Ideas</h3>
          <select
        ...
        <div className="space-y-2">
          {filteredIdeas.map((idea, index) => (
            <div key={idea.id} className="bg-card border border-border rounded p-4 flex items-center gap-4">
              <div className="text-[20px] font-mono text-muted-foreground/40 w-8 flex-shrink-0">
                {String(index + 1).padStart(2, "0")}.
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-[14px] font-semibold text-foreground">{idea.name}</span>
                  <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] font-medium text-muted-foreground">
                    {idea.category}
                  </span>
                  <span className="text-[11px] text-muted-foreground">{idea.source}</span>
                </div>
                <p className="text-[12px] text-muted-foreground truncate">{idea.description}</p>
              </div>
              <div className={`text-[18px] font-mono font-semibold flex-shrink-0 ${getScoreColor(idea.score)}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded p-5 shadow-sm">
          <h3 className="text-[13px] font-semibold text-foreground mb-4">Signal Source Distribution</h3>
          <div className="space-y-3">
            {sourceData.map((source) => (
              <div key={source.source} className="flex items-center gap-3">
                <div className="w-24 text-[13px] font-medium text-foreground">{source.source}</div>
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[12px] text-muted-foreground font-mono">{source.count} signals</span>
                    <span className="text-[12px] text-muted-foreground font-mono">{source.percentage}%</span>
                  </div>
                  <div className="w-full bg-muted rounded-full h-[3px]">
                    <div
                      className="bg-primary h-[3px] rounded-full transition-all"
        ...
        <div className="bg-card border border-border rounded p-5 shadow-sm">
          <h3 className="text-[13px] font-semibold text-foreground mb-4">Signal Velocity</h3>
          <div className="space-y-2">
            {sourceData.map((source) => (
              <div key={source.source} className="flex items-center justify-between p-3 bg-muted/40 rounded">
                <div>
                  <div className="text-[13px] font-medium text-foreground">{source.source}</div>
                  <div className="text-[12px] text-muted-foreground font-mono">
                    {source.velocity.toFixed(1)} signals/day
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[18px] font-mono font-semibold text-primary">
      ...
      {/* Market Summary */}
      <div className="bg-card border border-border rounded p-5 shadow-sm">
        <h3 className="text-[13px] font-semibold text-foreground mb-4">Market Summary ({timeRange})</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="border-t-2 border-primary pt-3">
            <div className="text-[28px] font-mono font-semibold text-primary mb-0.5">
              {sourceData.reduce((sum, source) => sum + source.count, 0)}
            </div>
            <div className="text-[12px] text-muted-foreground">Total Signals</div>
          </div>

          <div className="border-t-2 border-primary pt-3">
            <div className="text-[28px] font-mono font-semibold text-[var(--positive)] mb-0.5">
              {sectorData.filter(s => s.momentum >= 70).length}
            </div>
            <div className="text-[12px] text-muted-foreground">Hot Sectors</div>
          </div>

          <div className="border-t-2 border-primary pt-3">
            <div className="text-[28px] font-mono font-semibold text-[var(--warning)] mb-0.5">
              {topIdeas.filter(idea => idea.score >= 8.0).length}
            </div>
            <div className="text-[12px] text-muted-foreground">High-Score Ideas</div>
          </div>

          <div className="border-t-2 border-primary pt-3">
            <div className="text-[28px] font-mono font-semibold text-foreground mb-0.5">
              {sourceData.reduce((sum, source) => sum + source.velocity, 0).toFixed(1)}
            </div>
            <div className="text-[12px] text-muted-foreground">Signals/Day</div>
          </div>
        </div>
      </div>
      ...
      {(userRole === "Admin" || userRole === "Analyst") && (
        <div className="bg-card border border-border rounded p-4">
          <h3 className="text-[13px] font-semibold text-foreground mb-3">Export & Analysis</h3>
          <div className="flex gap-2">
            <button className="px-4 py-2 bg-primary text-primary-foreground rounded text-[13px] font-medium hover:bg-primary/90 transition-colors duration-150">
              Export Trend Report
            </button>
            <button className="px-4 py-2 bg-secondary text-secondary-foreground rounded text-[13px] font-medium hover:bg-secondary/80 transition-colors duration-150">
              Schedule Alert
            </button>
            <button className="px-4 py-2 bg-secondary text-secondary-foreground rounded text-[13px] font-medium hover:bg-secondary/80 transition-colors duration-150">
              Compare Periods
