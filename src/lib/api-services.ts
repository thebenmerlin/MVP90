...
// Handles connections to GitHub, Product Hunt, Supabase, and other external APIs
...
// Supabase Service for user data and analytics
...
export class MetricsCalculator {
  ...
  static calculateOriginalityScore(description: string, tags: string[]): number {
    // Simple keyword uniqueness calculation
  ...
  static calculateReplicabilityScore(
    ...
    // Higher tech complexity = lower replicability
    score -= techComplexity * 10;
    
    // Larger team = lower replicability
    score -= Math.min(teamSize * 5, 25);
    
    // Patents reduce replicability
  ...
  static calculateFreshnessScore(createdAt: string, lastUpdate: string): number {
    ...
    // Newer projects get higher scores, recent updates boost score
